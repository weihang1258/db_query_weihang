...
- ✅ 前端弹窗询问 → 选择格式 → 文件下载完整流程（该弹窗已按反馈移除）
- ✅ `/export-db-query` 命令脚本化后实测通过：
  ...
  - 不存在的库 / 无参数 → 均非 0 退出码 + 明确错误信息
  - 自然语言描述带空格可完整保留（参数重新拼接）
- ⚠️ 格式参数说明：脚本只把恰好为 `csv`/`json` 的第 2 参数识别为格式，**其他值会被当作查询的一部分**（支持省略格式的自然语言输入）；格式合法性最终由后端接口校验（非 `csv`/`json` 返回 400）
